...
const PROGRESS_PATH = path.join(__dirname, '..', 'scripts', '.regenerate-all-progress.json');
...
async function main() {
  ...
  console.log(`共有 ${questions.length} 道题需要生成解析，已完成 ${completed.size} 道`);
  ...
  for (let i = 0; i < remaining.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      for (const q of batch) {
        try {
          ...
          console.error(`单题 ${q.type}-${q.id} 失败:`, singleErr.message);
          results.push({});
        }
      }
    }
    ...
    console.log(`已完成 ${completed.size}/${questions.length} 道题`);
